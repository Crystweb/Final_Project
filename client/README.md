...
This will make `moduleA.js` and all its unique dependencies as a separate chunk that only loads after the author clicks the 'Load' button.
 ...
 * On mobile devices, your app can be added directly to the author's home screen, app icon and all. You can also re-engage users using web **push notifications**. This eliminates the need for the app store.
...
After the author visits a page that has `unregister()`,
...
[let the author know](https://developers.google.com/web/fundamentals/instant-and-offline/offline-ux#inform_the_user_when_the_app_is_ready_for_offline_consumption)
...
When a author adds a web app to their homescreen using Chrome or Firefox on
...
For example:
```js
<BrowserRouter basename="/alendar"/>
<Link to="/today"/> // renders <a href="/alendar/today">
```
...
If you are deploying to a GitHub author page instead of a project page you'll need to make two
...
GitHub Pages doesn’t support routers that use the HTML5 `pushState` history API under the hood (for example, React Router using `browserHistory`). This is because when there is a fresh page load for a url like `http://author.github.io/todomvc/todos/42`, where `/todos/42` is a frontend route, the GitHub Pages server returns 404 because it knows nothing of `/todos/42`. If you want to add a router to a project hosted on GitHub Pages, here are a couple of solutions:

* You could switch from using HTML5 history API to routing with hashes. If you use React Router, you can switch to `hashHistory` for this effect, but the URL will be longer and more verbose (for example, `http://author.github.io/todomvc/#/todos/42?_k=yknaj`). [Read more](https://reacttraining.com/react-router/web/api/Router) about different history implementations in React Router.
...
2. `git remote set-url origin https://<author>:<token>@github.com/<author>/<repo>` .
