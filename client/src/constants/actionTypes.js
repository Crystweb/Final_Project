export const ADD_SHIFT = 'ADD_SHIFT'

export const GET_SELECTED_DATE = 'GET_SELECTED_DATE'

export const GET_CURRENT_USER = 'GET_CURRENT_USER'

export const GET_ALL_POSITIONS = 'GET_ALL_POSITIONS'

export const GET_COMMETNS_FOR_SELECTED_DATE = 'GET_COMMENTS_FOR_SELECTED_DATE'

export const GET_SCHEDULES = 'GET_SCHEDULES'

export const GET_LOCATIONS = 'GET_LOCATIONS'

export const GET_ALL_VACANCIES= 'GET_ALL_VACANCIES'

export const ADD_NEW_VACANCY= 'ADD_NEW_VACANCY'

export const UPDATE_VACANCY= 'UPDATE_VACANCY'

export const GET_TASK_STATUSES = 'GET_TASK_STATUSES'

export const GET_TASKS = 'GET_TASKS'

export const GET_FREQUENCIES = 'GET_FREQUENCIES'

export const GET_ALL_USERS = 'GET_ALL_USERS'

export const ADD_NEW_TASK = 'ADD_NEW_TASK'

export const DELETE_CLOSED_TASK = 'DELETE_CLOSED_TASK'

export const ADD_TASKS_FOR_HISTORY = 'ADD_TASKS_FOR_HISTORY'

export const DELETE_COMMENT = 'DELETE_COMMENT'

export const ADD_NEW_COMMENT = 'ADD_NEW_COMMENT'

export const UPDATE_COMMENT = 'UPDATE_COMMENT'

export const DELETE_CALENDAR_DATE = 'DELETE_CALENDAR_DATE'

export const ADD_ROOM_CHECK_HISTORY = 'ADD_ROOM_CHECK_HISTORY'

export const SAVE_CURRENT_FLOOR = 'SAVE_CURRENT_FLOOR'

export const DELETE_CURRENT_FLOOR = 'DELETE_CURRENT_FLOOR'

export const USER_DOWNLOAD = 'USER_DOWNLOAD'

export const SAVE_TOKEN = 'SAVE_TOKEN'
