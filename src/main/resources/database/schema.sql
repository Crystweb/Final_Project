...
CREATE TABLE IF NOT EXISTS `location` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `l_title` VARCHAR(50) NOT NULL UNIQUE,
  PRIMARY KEY (`id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8;
...
CREATE TABLE IF NOT EXISTS `task` (
  ...
  `t_status` ENUM ('ACTIVE', 'CLOSED', 'PROCESSING', 'FINISHED'),
  `t_text` VARCHAR(500) NOT NULL,
  `t_created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `t_status_changed` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
